...
function displayMessage(mode, text)
  ...
  if msgtype.screenTarget then
    if string.find(text, '12//,') or string.find(text, '12|,') or string.find(text, 'p#,') or string.find(text, '12&,') or string.find(text, '#getSto#,') then
    return     --alterado!
    end   
	if string.find(text, '#ph#') then
	return
	end
	if string.find(text, '#getSto#') then
	return
	end
